...
- `-o`/`--output FILE` also writes a CSV report (`package,version,cve,poc_found,poc_repo,poc_stars,poc_url,epss,privesc,skip_reason`). `poc_found` is `yes`, `no`, `error` (the GitHub lookup failed — rate-limited or network error, unverified not confirmed absent), or `skipped` (see `--min-epss`/`--privesc` below, `skip_reason` says which).
