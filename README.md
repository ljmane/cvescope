...
**Usage:**<br>
`./scan_packages.sh [-f|--file FILE] [-o|--output CSV_FILE] [-d|--delay SECONDS] [-x|--exclude REGEX] [-X|--exclude-file FILE] [-s|--since WHEN] [-p|--poc-only] [--min-epss N] [--privesc] [--no-cache] [--cache-ttl HOURS] [--stale-out FILE]`<br>
...
- `-o`/`--output FILE` also writes a CSV report (`package,version,cve,poc_found,poc_repo,poc_stars,poc_url,epss,privesc,skip_reason`) — a complete record independent of what's shown on screen, so `skipped` rows (see `--min-epss`/`--privesc` below) always land here even though they're silent on the console. `poc_found` is `yes`, `no`, `error` (the GitHub lookup failed — rate-limited or network error, unverified not confirmed absent), or `skipped`.
- `-p`/`--poc-only` only reports CVEs that have a public PoC (skips `no`; `error` still shows on the console, since that's "unchecked," not "confirmed clean").
...
- `--stale-out FILE` writes that same `package version` list to `FILE`, in exactly the format this script reads as input — so instead of re-running `-s all` against your whole package list (slow), you can re-check just the ones that actually need it:<br>
  ...
  ```
- `--privesc` skips the GitHub PoC lookup for any CVE not classified as local privilege escalation — it's still counted in the summary and written to CSV (`-o`) as `skipped`, but doesn't print a line on the console, so a package where every CVE gets skipped (and its `[i/N] package version` header) stays completely silent rather than filling the screen with `PoC: SKIPPED` noise. Classification is a heuristic computed at zero extra API cost from the same NVD response already fetched: the CVSS vector must be local/physical attack vector (`AV:L`/`AV:P`) *and* the description must use actual escalation language ("privilege escalation," "gain root," "SUID," etc.) — a local DoS or info leak is `AV:L` too but doesn't count on its own. Favors recall over precision by design (may over-classify some non-privesc local bugs), consistent with this tool's general bias toward not hiding things. **Off by default.**
- `--min-epss N` (e.g. `0.1`) skips the GitHub PoC lookup for any CVE whose [EPSS score](https://www.first.org/epss/) (FIRST.org's predicted probability of real-world exploitation in the next 30 days, 0–1) is below `N` — same silent-skip behavior as `--privesc` above (counted, in CSV, no console line). CVEs with no EPSS data yet, or that this scan classifies as privesc (see `--privesc` above), are always checked regardless of score. **Off by default.**<br>
  ⚠️ EPSS is trained mostly on internet-facing, remotely-exploitable vulnerabilities — local-privilege-escalation bugs (like the PackageKit example used throughout this README, EPSS 0.0046 despite a 100+-star PoC) score low almost by nature, regardless of whether a real, working PoC exists. The privesc exemption above catches the clear-cut cases, but the classifier is a heuristic and won't catch every real privesc CVE — if that's specifically what you're after, `--privesc`, `-s`/`--since`, and `-x`/`-X` are safer ways to cut volume than `--min-epss` alone.
- `--no-cache` / `--cache-ttl HOURS` control the local results cache (on by default, 24h freshness). Each `package+version+since+min-epss+privesc` combination is cached — including packages with zero matching CVEs, since that's the common case — in a single SQLite database at `~/.cache/cvescope/cache.db` (override the directory with `CVESCOPE_CACHE_DIR`; requires `sqlite3`, skip that dependency entirely with `--no-cache`). A repeat scan of an unchanged package list within the TTL window does no NVD/GitHub calls at all for anything already cached. Pass `--no-cache` to force a fully live check, or lower `--cache-ttl` if you want fresher data sooner.
- `-d`/`--delay SECONDS` overrides the pause between NVD/GitHub/EPSS API calls. Scanning a full package list means one NVD request per uncached package plus one GitHub request per CVE found (unless skipped by `--min-epss`/`--privesc`), so this respects both services' unauthenticated rate limits by default (and paces faster automatically once `GITHUB_TOKEN`/`NVD_API_KEY` are set) — expect a full, uncached system scan to take a while without both tokens, even with a `--stale-out` list, if several packages have multiple CVEs each. Caching and `--min-epss`/`--privesc` are the levers that actually cut down the number of GitHub calls made in the first place (1-per-CVE-found drops to 1-per-classified/scored-CVE); `-d` only controls how fast the remaining ones fire.
